...
- 项目已加入测试入口 `run-tests.bat`，当前覆盖上下文、长期项目记忆、风险策略、命令风险分类、验证计划、运行日志、运行日志查看器、运行自检、最终回复可信度、UI 运行调试入口、Agent 流式聚合、工具展示、工具输出压缩、错误恢复、任务规划、失败定位、失败聚焦读取、增量验证、影响分析、项目地图、符号搜索、变更计划、Patch 失败恢复、修复策略、防循环和增强回滚。
