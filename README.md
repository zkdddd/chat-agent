...
- 普通聊天使用流式输出；代码 Agent 当前是同步模型调用，但 UI 会持续展示阶段状态和工具报告。
...
- Agent 支持轻量任务规划，会在运行前生成检查清单，并在检查、修改、验证、总结时更新步骤状态。
...
- Agent 支持 Python 符号级搜索，可以按 class、function、method、import 查找定义位置。
...
- 项目已加入测试入口 `run-tests.bat`，当前覆盖上下文、风险策略、验证计划、运行日志、运行日志查看器、运行自检、工具展示、工具输出压缩、错误恢复、任务规划、失败定位、失败聚焦读取、增量验证、影响分析、项目地图、符号搜索、变更计划、Patch 失败恢复、修复策略和防循环。
